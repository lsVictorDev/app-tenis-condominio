...
class _ReservationPageState extends State<ReservationPage> {
  ...
  Future<void> _carregarHorariosOcupados() async {
    ...
    try {
      final dataReserva = DateFormat("yyyy-MM-dd").format(_dataSelecionada);
      ...
      for (final reserva in reservas.docs) {
        ...
        final horaInicio = dados["horaInicio"] as String;

        final horaFim = dados["horaFim"] as String;

        final inicio = int.parse(horaInicio.split(":")[0]);

        final fim = int.parse(horaFim.split(":")[0]);

        for (int hora = inicio; hora < fim; hora++) {
          horariosOcupados.add("${hora.toString().padLeft(2, '0')}:00");
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Não foi possível verificar os horários: $e")),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _reservarQuadra() async {
    ...
    try {
      ...
      for (final reserva in reservas.docs) {
        ...
        final horarioExistente = dados["horaInicio"] as String;

        final horarioFimExistente = dados["horaFim"] as String;

        final inicioExistente = int.parse(horarioExistente.split(":")[0]);

        final fimExistente = int.parse(horarioFimExistente.split(":")[0]);
  ...
  Widget build(BuildContext context) {
    ...
    final horaSelecionada = int.parse(_horarioSelecionado.split(":")[0]);
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: _quadras.map((quadra) {
                return DropdownMenuItem(value: quadra, child: Text(quadra));
              }).toList(),
              onChanged: (value) {
            ...
            if (_carregandoHorarios)
              ...
              GridView.builder(
                ...
                itemBuilder: (context, index) {
                  return _horarioButton(_horarios[index]);
                },
              ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _reservando ? "VERIFICANDO..." : "CONFIRMAR RESERVA",
                ...
                onPressed: _reservando ? null : _reservarQuadra,
              ),
            ),
            ...
            Row(
              ...
              children: [
                ...
                Icon(Icons.circle, size: 12, color: Colors.red.shade700),
                const SizedBox(width: 5),
                const Text("Ocupado"),
              ],
            ),
